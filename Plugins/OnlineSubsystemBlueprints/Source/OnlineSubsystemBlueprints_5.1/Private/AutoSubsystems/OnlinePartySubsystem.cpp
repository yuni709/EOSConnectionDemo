...
void UOnlinePartySubsystemRejoinParty::Activate()
{
    ...
    if (!(ValidateArrayElements<FUniqueNetIdRepl>(this->__Store__FormerMembers, [](const FUniqueNetIdRepl& Val) { return Val.IsValid() && Val.GetUniqueNetId().IsValid(); })))
    {
...
UOnlinePartySubsystemCancelInvitation* UOnlinePartySubsystemCancelInvitation::CancelInvitation(UOnlinePartySubsystem* Subsystem ,FUniqueNetIdRepl LocalUserId, FUniqueNetIdRepl TargetUserId, UPartyId* PartyId)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("OnlinePartySubsystem is not available. It's possible the online subsystem you are using doesn't support it. Alternatively, make sure that you are not attempting to use Online Subsystem Blueprints during the Init event in a GameInstance. Add a 'Delay 0.1' node between the Init event and the rest of your logic if you need to use Online Subsystem Blueprints this early in game startup."));
...
void UOnlinePartySubsystemCancelInvitation::Activate()
{
    ...
    auto __DelegateHandle_OnCancelPartyInvitationComplete = FOnCancelPartyInvitationComplete::CreateUObject(this, &UOnlinePartySubsystemCancelInvitation::HandleCallback_OnCancelPartyInvitationComplete);

    Handle->CancelInvitation(this->__Store__LocalUserId.GetUniqueNetId().ToSharedRef().Get(), this->__Store__TargetUserId.GetUniqueNetId().ToSharedRef().Get(), this->__Store__PartyId->ToNative(), __DelegateHandle_OnCancelPartyInvitationComplete);
}

void UOnlinePartySubsystemCancelInvitation::HandleCallback_OnCancelPartyInvitationComplete(const FUniqueNetId& SenderUserId, const FOnlinePartyId& PartyId, const FUniqueNetId& TargetUserId, const FOnlineError& Result)
{
    if (!IsValid(this->__Store__Subsystem))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Subsystem was not valid during callback handling"));
    ...
    this->OnCancelPartyInvitationComplete.Broadcast(ConvertDangerousUniqueNetIdToRepl(SenderUserId), UPartyId::FromNative(PartyId), ConvertDangerousUniqueNetIdToRepl(TargetUserId), FOnlineErrorInfo::FromNative(Result));
}
